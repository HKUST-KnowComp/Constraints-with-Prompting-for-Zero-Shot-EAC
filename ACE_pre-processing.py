...
root_dir = "/home/data/zlinai/Event-Centric_NLP_1"
os.chdir(root_dir)
...
entity_type_mapping = {"Crime": "CRIME",
                    "Sentence": "SEN",
                    "Numeric": "NUM",
                    "Job-Title": "JOB"}
...
def sent_tokenize(text: Tuple[str, int, int]) -> List[Tuple[str, int, int]]:
...
def read_sgm_file(path: str) -> List[Tuple[str, int, int]]:
...
def tokenize(sentence: Tuple[str, int, int],
             entities: List[Entity],
             events: List[Event]) -> List[Tuple[int, int, str]]:
...
def convert(sgm_file: str,
            apf_file: str,
            time_and_val: bool = False) -> Document:
    """Converts a document.
    ...
    """
    sentences = read_sgm_file(sgm_file)
    ...
    '''
    # Tokenization
    sentence_tokens = [tokenize(s, ent, evt) for s, ent, evt
                       in zip(sentences, sentence_entities, sentence_events)]
    '''

    # Convert span character offsets to token indices
    sentence_objs = []
    for i, (ents, evts, rels, sent) in enumerate(zip(
            sentence_entities, sentence_events,
        ...
        for line in r:
            ...
            for sentence in doc['sentences']:
                ...
                entity_span = {e['mention_id']: {'start': e['start'], 'end': e['end']}
                               for e in sentence['entities']}
                entity_types = {e['mention_id']: e['entity_type']
                               for e in sentence['entities']}
                ...
                for event in sentence['events']:
                    ...
                    for arg in event['arguments']:
                        arg['text'] = re.sub('[{}]', "", entity_text[arg['mention_id']])
                        arg['start'] = entity_span[arg['mention_id']]['start'] - sentence['start']
                        arg['end'] = entity_span[arg['mention_id']]['end'] - sentence['start']
                        
                        #fix inconsistency in argument offset
                        if((doc['doc_id'] == 'alt.books.tom-clancy_20050130.1848' and sentence['sent_id'] == 'alt.books.tom-clancy_20050130.1848-5') \
                            or (doc['doc_id'] == 'AGGRESSIVEVOICEDAILY_20041208.2133' and sentence['sent_id'] == 'AGGRESSIVEVOICEDAILY_20041208.2133-2' and not (arg['start'] == 18 and arg['end'] == 33)) \
                            or (doc['doc_id'] == 'APW_ENG_20030325.0786' and sentence['sent_id'] == 'APW_ENG_20030325.0786-4')
                            ):
                            arg['start']-=4
                            arg['end']-=4
                        ...
                        if(doc['doc_id'] == 'APW_ENG_20030422.0469' and sentence['sent_id'] == 'APW_ENG_20030422.0469-30' \
                            and arg['start'] == 137 and arg['end'] == 146):
                            arg['end']-=1

                        if(doc['doc_id'] == 'APW_ENG_20030519.0548' and sentence['sent_id'] == 'APW_ENG_20030519.0548-5' \
                            and arg['start'] == 21 and arg['end'] == 37):
                            arg['end']-=1



                        arg['role'] = simplify_arg_role_name(arg['role'])
                        temp_entity_type = entity_types[arg['mention_id']]
                        if(temp_entity_type in entity_type_mapping):
                            temp_entity_type = entity_type_mapping[temp_entity_type]
                        arg['entity_type'] = 'TIME' if arg['role'] == 'Time' else temp_entity_type
                ...
                for event in sentence['events']:
                    events.append({
                        ...
                        'trigger': {
                            ...
                            "start": event['trigger']["start"] - sentence['start'],
                            "end": event['trigger']["end"] - sentence['start']
                        },
                ...
                sent_obj = {
                    ...
                    'sentence': re.sub('[{}]', "", sentence['text']),
                    'event_mentions': events
                }
                w.write(json.dumps(sent_obj) + '\n')
    print('skip sent num: {}'.format(skip_sent_num))
...
def split_data(input_file: str,
    ...
    with open(input_file, 'r', encoding='utf-8') as r, \
        ...
        open(os.path.join(output_dir, 'merge.event.json'), 'w') as w_merge:
...
if __name__ == '__main__':
    ...
    #input_dir = os.path.join(args.input, 'English')

    # Convert to doc-level JSON format
    json_path = os.path.join(args.output, 'english.json')
    convert_batch(args.input, json_path, time_and_val=args.time_and_val)

    # Convert to event-only format
    event_path = os.path.join(args.output, 'english.event.json')
